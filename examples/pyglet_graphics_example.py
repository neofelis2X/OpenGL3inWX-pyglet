...
import pyglet
from pyglet.gl import Config, GL_CULL_FACE, GL_DEPTH_TEST, \
                      GL_TRIANGLES, glClearColor, glEnable
from pyglet.math import Mat4, Vec3

WINDOW_X = WINDOW_Y = 480
COL_BG = (96, 147, 172)
COL_TOR = (246, 120, 40)
...
try:
    ...
    window = pyglet.window.Window(width=WINDOW_X, height=WINDOW_Y, resizable=True, config=config)
except pyglet.window.NoSuchConfigException:
    # Fall back to no multisampling if not supported
    print("Had to use fallback.")
    window = pyglet.window.Window(width=WINDOW_X, height=WINDOW_Y, resizable=True)
...
def on_resize(width, height):
    window.viewport = (-width / 2, -height / 2, *window.get_framebuffer_size())
    window.projection = Mat4.perspective_projection(window.aspect_ratio,
                                                    z_near=0.1, z_far=255, fov=55)
...
def create_torus(radius, inner_radius, slices, inner_slices, shader, batch):
    ...
    diffuse = [COL_TOR[0] / 255, COL_TOR[1] / 255, COL_TOR[2] / 255, 1.0]
